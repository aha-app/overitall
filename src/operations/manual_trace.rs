...
pub fn start_recording(app: &mut App) {
    app.manual_trace_recording = true;
    app.manual_trace_start = Some(Local::now());
...
pub fn stop_recording(app: &mut App, manager: &ProcessManager) -> Result<String, String> {
    let start_time = app.manual_trace_start
    ...
    if filtered.is_empty() {
        // Reset state
        app.manual_trace_recording = false;
        app.manual_trace_start = None;
        return Err("No logs captured in time window".to_string());
    }

    // Enter trace filter mode
    app.create_snapshot(filtered);
    app.trace_time_start = Some(start_time);
    app.trace_time_end = Some(end_time);
    app.active_trace_id = None; // No correlation ID for manual traces
